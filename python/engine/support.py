...
def write_support_pack(extra: dict[str, Any] | None = None) -> Path:
    ...
    lines.extend(
        [
            ...
            "",
            "Offline / enterprise media",
            "-" * 40,
            "MAGIC_ENTERPRISE_ISO_DIR = folder with VL/enterprise ISOs (searched before CDN)",
            "MAGIC_DU_CAB_DIR = optional Dynamic Update .cab folder (place next to ISO; Setup uses /dynamicupdate enable when online)",
            "If offline: copy latest Safe OS / SSU / LCU cabs into MAGIC_DU_CAB_DIR and keep network off only after staging.",
            "",
